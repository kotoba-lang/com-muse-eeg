(ns muse_eeg.main
  "Kotodama WASM entrypoint for the Muse_eeg clean-room actor (L4) — Clojure port.
  ...
  facts (`muse_eeg.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "muse_eeg")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "museeeg_dev"
    :fields [:model :channels :samplingHz :firmware]  :required [:model :channels]
    :coerce {:channels :int :samplingHz :int}  :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "museeeg_ses"
    :fields [:deviceId :subjectRef :startedAt :durationSec] :required [:startedAt :durationSec]
    :coerce {:durationSec :int}             :refs {:deviceId "Device"}}
   {:entity "Channel"     :plural "channels"     :id-prefix "museeeg_cha"
    :fields [:deviceId :label :location :impedanceKohm]  :required [:label :location]
    :coerce {:impedanceKohm :float}         :refs {:deviceId "Device"}}
   {:entity "Signal"      :plural "signals"      :id-prefix "museeeg_sig"
    :fields [:sessionId :channelLabel :band :dataRef]  :required [:channelLabel :band]
    :coerce {}                              :refs {:sessionId "Session"}}
   {:entity "Event"       :plural "events"       :id-prefix "museeeg_eve"
    :fields [:sessionId :marker :offsetMs]      :required [:marker :offsetMs]
    :coerce {:offsetMs :int}                :refs {:sessionId "Session"}}
   {:entity "Calibration" :plural "calibrations" :id-prefix "museeeg_cal"
    :fields [:deviceId :type :completedAt]      :required [:type :completedAt]
...
(defn emit-facts
  "EAVT facts for one record: {\"muse_eeg.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "muse_eeg-compat" :tier tier :entities entities} 200])
...
(defn start! [] :muse_eeg-compat/ready)
